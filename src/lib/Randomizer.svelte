<script>
  ...
  let rolledCards = $state(stickerSheets.filter(({id}) => initialRolledIndices.includes(id))) 

  const rollNewCards = () => {
    const rolledIndices = pickN(appState.pickedOptions,3)

    rolledCards = stickerSheets.filter(({id}) => rolledIndices.includes(id)).sort(() => Math.random() > 0.5 ? 1 : -1)
  }
</script>
...
<StickerCard card={card}></StickerCard>
{/each}
</div>
<button onclick={rollNewCards}>
Randomize
</button>
</div>
